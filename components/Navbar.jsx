    ...
    <nav className="bg-[#0B4228] text-white shadow-2xl sticky top-0 z-50 transition-colors duration-300 border-b-2 border-[#09361a]">
        ...
        <div className="hidden md:flex items-center gap-6 lg:gap-8 text-[15px] font-bold justify-end py-2 ml-4">
          ...
          {!(role === 'admin' || role === 'sub-admin') && (
            <>
              ...
              <Link href="/map" className={isActive('/map')}>
                Park Map
              </Link>
            </>
          )}
      ...
      >
        <div className="py-2 flex flex-col">
          {!isAuthenticated && (
            <Link href="/" className={isActive('/', true)}>
              Home
            </Link>
          )}
          {!(role === 'admin' || role === 'sub-admin') && (
            <>
              {!isAuthenticated && (
                <Link href="/about" className={isActive('/about', true)}>
                  About Us
                </Link>
              )}
              <Link href="/map" className={isActive('/map', true)}>
                Park Map
              </Link>
            </>
          )}
          {isAuthenticated && (
            <>
              <Link href="/book" className={isActive('/book', true)}>
                Book Tickets
              </Link>
              {!(role === 'admin' || role === 'sub-admin') && (
                <Link href="/rewards" className={isActive('/rewards', true)}>
                  Play & Win
                </Link>
              )}
