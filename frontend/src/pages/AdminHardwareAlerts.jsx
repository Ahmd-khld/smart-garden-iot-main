...
const AdminHardwareAlerts = () => {
  ...
  useEffect(() => {
    const fetchAlerts = async (pageNum, silent = false) => {
  ...
  }, [page, filterDate, filterType, navigate]);
  ...
  const handleExportCSV = () => {
    ...
    link.href = url;
    link.setAttribute('download', `smart-park-hardware-alerts-${new Date().toISOString().split('T')[0]}.csv`);
        ...
        <div className="bg-white dark:bg-gray-800 rounded-[40px] shadow-2xl border border-smart-light/10 dark:border-gray-700 overflow-hidden flex flex-col h-[80vh]">
          <div className="bg-smart-bg dark:bg-gray-900 px-8 py-6 border-b border-smart-light/10 flex justify-between items-center">
            <h2 className="text-xl font-black text-smart-dark dark:text-white flex items-center tracking-tighter uppercase italic">
              <svg className="w-6 h-6 mr-3 text-smart-light" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
            ...
            <div className="flex items-center space-x-3">
              ...
              {(filterDate || filterType !== 'all') && (
                <button onClick={() => { setFilterDate(''); setFilterType('all'); setPage(1); }} className="px-3 py-2 bg-red-50 dark:bg-red-900/30 hover:bg-red-100 dark:hover:bg-red-900/50 text-red-600 dark:text-red-400 rounded-xl font-bold text-[10px] uppercase tracking-widest transition-colors border border-red-200 dark:border-red-800">Clear</button>
              )}
            </div>
          </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead className="sticky top-0 bg-smart-bg dark:bg-gray-900 z-10">
                  <tr className="border-b border-smart-light/10 text-smart-gray dark:text-gray-500 text-[10px] font-black uppercase tracking-widest">
                    <th className="px-4 py-3 pl-8 whitespace-nowrap text-left w-1/4">Date &amp; Time</th>
                  ...
                  {filteredAlerts.map(alert => (
                    <tr key={alert._id || alert.id} className="hover:bg-smart-bg/50 dark:hover:bg-gray-700/50 transition-colors">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap align-top">
                        {alert.type === 'warning' && <span className="bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-400 text-[9px] font-black px-2 py-1 rounded-md uppercase tracking-wider border border-yellow-200 dark:border-yellow-800 inline-block w-[72px] text-center">Warning</span>}
                        {alert.type === 'info' && <span className="bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-400 text-[9px] font-black px-2 py-1 rounded-md uppercase tracking-wider border border-blue-200 dark:border-blue-800 inline-block w-[72px] text-center">Info</span>}
                        {alert.type === 'action' && <span className="bg-smart-light/10 dark:bg-smart-light/20 text-smart-dark dark:text-smart-glow text-[9px] font-black px-2 py-1 rounded-md uppercase tracking-wider border border-smart-light/20 inline-block w-[72px] text-center">Action</span>}
                        {alert.type === 'success' && <span className="bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400 text-[9px] font-black px-2 py-1 rounded-md uppercase tracking-wider border border-green-200 dark:border-green-800 inline-block w-[72px] text-center">Success</span>}
                        {alert.type === 'error' && <span className="bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-400 text-[9px] font-black px-2 py-1 rounded-md uppercase tracking-wider border border-red-200 dark:border-red-800 inline-block w-[72px] text-center">Error</span>}
                      </td>
                      <td className="px-4 py-3 text-smart-dark dark:text-gray-200 font-medium text-sm leading-relaxed break-words align-top">{alert.message}</td>
                    </tr>
                  ))}
            ...
            {totalPages > 1 && !loading && (
              <div className="bg-smart-bg/30 dark:bg-gray-900/30 px-6 py-4 border-t border-smart-light/10 flex flex-col sm:flex-row justify-between items-center gap-4 mt-auto">
                <span className="text-[10px] font-bold text-smart-gray dark:text-gray-400 uppercase tracking-widest text-center sm:text-left w-full sm:w-1/3">
                  Showing {(page - 1) * 50 + 1} to {Math.min(page * 50, totalAlertsCount)} of {totalAlertsCount}
                </span>
