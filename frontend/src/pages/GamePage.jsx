                ...
                <div className="max-w-md text-center p-10 bg-white dark:bg-gray-800 rounded-3xl shadow-2xl border-2 border-smart-glow/20">
                    ...
                    {leaderboard.length > 0 && (
                        <div className="mt-8 border-t border-gray-200 dark:border-gray-700 pt-6 w-full">
                            <h3 className="text-sm font-black mb-4 dark:text-white uppercase tracking-widest text-smart-light">Top Players</h3>
                            <div className="space-y-2 text-sm text-left">
                                {leaderboard.map((user, idx) => (
                                    <div key={user._id || idx} className="flex justify-between items-center bg-gray-50 dark:bg-gray-700/50 px-4 py-3 rounded-xl border border-gray-100 dark:border-gray-600">
                                        <span className="font-bold text-gray-700 dark:text-gray-300 capitalize">{idx + 1}. {user.name}</span>
                                        <span className="font-black text-smart-light">{user.gameStats?.topScore || 0} <span className="text-[10px] text-gray-400">PTS</span></span>
                                    </div>
                                ))}
                            </div>
                        </div>
                ...
                <div className="relative">
                    ...
                    <GameArea>
                        {renderItems.map(item => (
                            <GameItem key={item.id} style={{ transform: `translate3d(${item.x}px, ${item.y}px, 0)` }}>
                                {item.emoji}
                            </GameItem>
                        ))}
