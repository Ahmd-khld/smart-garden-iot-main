...
const detectInsiderThreats = async (riskRegister = []) => {
  try {
    ...
    const sensitiveActionPatterns = [
      'Restricted user:',
      'Cleared security audit logs',
      'Cleared hardware alerts',
      'Deleted database backup',
      'Restored database from backup',
      'Manually reset park occupancy'
    ];
    const sensitiveRegex = new RegExp(sensitiveActionPatterns.join('|'), 'i');
    ...
    const suspiciousAdmins = await AdminAuditLog.aggregate([
      {
        $match: {
          ...
          $or: [
            { status: 'success', action: { $regex: sensitiveRegex } },
            { status: 'failed' }
          ]
        }
      },
