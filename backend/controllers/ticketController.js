...
const isDateValidForBooking = (date) => {
    ...
    maxDate.setDate(maxDate.getDate() + 30); // 30-day booking window
...
const checkout = async (req, res) => {
    try {
        ...
        if (subscriptionPlan === 'one-time') {
            ...
            if (userTicketsForDate + requestedCount > 200) {
                return res.status(400).json({
                    message: `You can only buy a maximum of 200 tickets per day. You already have ${userTicketsForDate} ticket(s) for this date.`
                });
            }
        } else if (subscriptionPlan === 'monthly') {
            if (requestedCount > 200) {
                return res.status(400).json({ message: 'You can only buy a maximum of 200 tickets per transaction.' });
            }
        }
