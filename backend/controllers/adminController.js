...
const isSuperAdmin = (req) => req.user && req.user.email === 'admin@smartpark.com';
...
const broadcastOccupancy = async (req) => {
    ...
    const capacityPercentage = Math.round((currentOccupancy / 200) * 100);
...
const getAdminStats = async (req, res) => {
    try {
        ...
        if (mostSoldAgg.length > 0) {
            ...
            const typeCap = top._id.type ? top._id.type.charAt(0).toUpperCase() + top._id.type.slice(1) : 'Unknown';
            mostSoldTicket = `${typeCap} (${top._id.plan || 'unknown'})`;
        }

        // Extract the counted value from the aggregation array safely
        const purchasingUsers = purchasingUsersAgg.length > 0 ? purchasingUsersAgg[0].totalPurchasingUsers : 0;
...
const scanTicket = async (req, res) => {
    try {
        ...
        if (scanStatus.lockoutUntil && new Date() < scanStatus.lockoutUntil) {
            ...
            return res.status(429).json({ message: `Scanner locked. Try again in ${remaining} minute(s).` });
        ...
        const handleFailure = async (alertMsg, alertType, resMsg, statusCode = 400) => {
            ...
            if (scanStatus.count >= 5) {
                ...
                failedScans.set(clientIp, scanStatus);
                await createHardwareAlert(req, 'SECURITY LOCKOUT: Gate scanner disabled for 5 minutes due to multiple failed scans.', 'error');
                return res.status(429).json({ message: 'Scanner locked due to multiple failed attempts. Try again in 5 minutes.' });
            }
            failedScans.set(clientIp, scanStatus);
        ...
        if (!mongoose.Types.ObjectId.isValid(ticketId)) {
            return handleFailure('Invalid QR format scanned at Gate.', 'error', 'Invalid Ticket ID format');
        }

        const ticket = await Ticket.findById(ticketId);

        if (!ticket) {
            return handleFailure('Unrecognized Ticket ID scanned at Gate.', 'error', 'Ticket not found', 404);
        }

        const now = new Date();
        if (ticket.validFrom && now < ticket.validFrom) {
            return handleFailure(`Premature entry attempt. Ticket valid from: ${new Date(ticket.validFrom).toLocaleDateString()}`, 'warning', `Ticket is not valid yet. Valid starting: ${new Date(ticket.validFrom).toLocaleDateString()}`);
        }
        if (ticket.validUntil && now > ticket.validUntil) {
            ...
            return handleFailure('Expired ticket scanned at Gate.', 'warning', `Ticket has expired on ${new Date(ticket.validUntil).toLocaleDateString()}`);
        }
        ...
        if (subType === 'monthly') {
            if (ticket.status !== 'active') {
                return handleFailure('Inactive or revoked monthly pass scanned at Gate.', 'error', 'Monthly ticket is not active or has been revoked.');
            }
            ...
            return handleSuccess('Monthly pass validated successfully at Gate.', 'Monthly Pass Validated');
        } else {
            // Default to 'one-time' behavior
            if (ticket.status === 'used') {
                return handleFailure('Duplicate entry attempt: Ticket already used.', 'warning', 'Ticket already scanned and used.');
            }
            ...
            if (ticket.status === 'active') {
                ...
                return handleSuccess('Ticket scanned successfully. Access granted.', 'Ticket scanned successfully. Access granted.');
            }
            
            return handleFailure('Invalid ticket status encountered at Gate.', 'error', 'Invalid ticket status');
        }
...
const getUsers = async (req, res) => {
    try {
        ...
        if (role === 'admin') {
            query.role = 'admin';
            // Super Admin can see ALL admins. Sub-admins cannot see the Super Admin.
            if (!isSuperAdmin(req)) query.email = { $ne: 'admin@smartpark.com' };
        } else if (role === 'user') {
            query.role = 'user';
        }
        ...
        if (role === 'admin') {
            const admins = await User.find(query).select('-password -savedCards').sort({ createdAt: -1 }).lean();
            return res.status(200).json({ users: admins });
        }

        const [users, total] = await Promise.all([
            User.find(query).select('-password -savedCards').sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
            User.countDocuments(query)
        ]);

        res.status(200).json({ users, totalUsers: total, currentPage: page, totalPages: Math.ceil(total / limit) || 1 });
    } catch (error) {
...
const toggleBlockUser = async (req, res) => {
    try {
        ...
        if (user.email === 'admin@smartpark.com') {
            return res.status(403).json({ message: 'The Super Admin account cannot be blocked.' });
        }
        
        if (user.role === 'admin' && !isSuperAdmin(req)) {
            return res.status(403).json({ message: 'Only the Super Admin can block or unblock sub-admins.' });
        }
        ...
        const io = req.app.get('io');
        if (io) {
            io.emit('userStatusUpdate', { userId: user._id, isBlocked: user.isBlocked });
        }

        await logAdminAction(req, `Toggled block status for user: ${user.email} (Blocked: ${user.isBlocked})`);
...
const createSubAdmin = async (req, res) => {
    try {
        ...
        if (!name || !email || !password || !ipAddress) {
            return res.status(400).json({ message: 'Name, email, password, and bound IP Address are required' });
        }
        ...
        if (ipExists) {
            return res.status(400).json({ message: 'This IP Address is already in use by another whitelist entry' });
        }
        ...
        if (io) {
            ...
            const safeUser = { _id: newAdmin._id, name: newAdmin.name, email: newAdmin.email, phone: newAdmin.phone, role: newAdmin.role, isBlocked: newAdmin.isBlocked, hasDisability: newAdmin.hasDisability, createdAt: newAdmin.createdAt };
...
const deleteUser = async (req, res) => {
    try {
        ...
        if (userCheck.email === 'admin@smartpark.com') {
        ...
        if (io) {
            ...
            const [totalTicketsSold, purchasingUsersAgg, mostSoldAgg, salesAgg] = await Promise.all([
                ...
                Ticket.aggregate([{ $group: { _id: { type: "$ticketType", plan: "$subscriptionPlan" }, count: { $sum: 1 } } }, { $sort: { count: -1 } }, { $limit: 1 }]),
                Ticket.aggregate([
                    { $match: { status: { $ne: 'cancelled' } } },
                    { $group: { _id: { year: { $year: "$createdAt" }, month: { $month: "$createdAt" } }, totalTickets: { $sum: 1 }, revenue: { $sum: "$price" } } },
            ...
            const formattedSales = salesAgg.map(s => ({ month: new Date(s._id.year, s._id.month - 1).toLocaleString('default', { month: 'short', year: 'numeric' }), totalTickets: s.totalTickets, revenue: s.revenue })).reverse();
...
const resetOccupancy = async (req, res) => {
    try {
        ...
        await logAdminAction(req, `Manually reset park occupancy. Archived ${result.modifiedCount} tickets.`);
        res.status(200).json({ 
...
const getHardwareAlerts = async (req, res) => {
    try {
        ...
        res.status(200).json({
            alerts, totalAlerts: total, currentPage: page, totalPages: Math.ceil(total / limit)
        });
    } catch (error) {
...
const getAuditLogs = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({
            logs, totalLogs: total, currentPage: page, totalPages: Math.ceil(total / limit)
        });
    } catch (error) {
...
const getBannedIPs = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({
            bannedIPs, totalBannedIPs: total, currentPage: page, totalPages: Math.ceil(total / limit) || 1
        });
    } catch (error) {
...
const getWhitelistedIPs = async (req, res) => {
    try {
        ...
        res.status(200).json({
            ips, totalIps: total, currentPage: page, totalPages: Math.ceil(total / limit) || 1
        });
    } catch (error) {
...
const removeWhitelistedIP = async (req, res) => {
    try {
        ...
        if (deleted.adminEmail) {
            const subAdminUser = await User.findOneAndDelete({ email: deleted.adminEmail, role: 'admin' });
            if (subAdminUser) {
                ...
                await logAdminAction(req, `Deleted sub-admin account (${deleted.adminEmail}) due to IP whitelist removal.`);
            }
        }
...
const getMonthlySales = async (req, res) => {
    try {
        ...
        const formattedSales = sales.map(s => ({
            month: new Date(s._id.year, s._id.month - 1).toLocaleString('default', { month: 'short', year: 'numeric' }),
...
const clearAuditLogs = async (req, res) => {
    try {
        ...
        await logAdminAction(req, `Cleared security audit logs${req.query.olderThan ? ` older than ${req.query.olderThan} days` : ' (All)'}`);
...
const clearHardwareAlerts = async (req, res) => {
    try {
        ...
        await logAdminAction(req, `Cleared hardware alerts${req.query.olderThan ? ` older than ${req.query.olderThan} days` : ' (All)'}`);
...
const createBackup = (req, res) => {
    ...
    exec(cmd, async (error, stdout, stderr) => {
        ...
        await logAdminAction(req, `Created manual database backup: smart-park-manual-${timestamp}.gzip`);
...
const downloadBackup = (req, res) => {
    try {
        ...
        if (!filename || !filename.endsWith('.gzip') || filename.includes('/') || filename.includes('\\')) {
...
const getPromoUsageStats = async (req, res) => {
    try {
        const stats = await PromoCode.aggregate([
            { $match: { isUsed: true } },
            { $group: { _id: "$code", count: { $sum: 1 }, totalDiscount: { $sum: "$discount" } } },
            { $sort: { count: -1 } },
            { $limit: 10 }
        ]);
        res.status(200).json(stats);
    } catch (error) {
        console.error('Fetch Promo Stats Error:', error);
        res.status(500).json({ message: 'Error fetching promo stats' });
    }
};
